...
********************************************************
This is the LICENSE-orig.txt file: 
 HIDAPI - Multi-Platform library for
 communication with HID devices.
...
static void register_error(hid_device *device, const char *op)
{
	...
	FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER |
		...
		(LPWSTR)((void *)&msg), 0/*sz*/,
		NULL);
	
	// Get rid of the CR and LF that FormatMessage() sticks at the
	// end of the message. Thanks Microsoft!
	ptr = (void *)op;	// dummy to stop -Wunused errors!!!
	ptr = msg;
	while (*ptr) {
		if (*ptr == '\r') {
			*ptr = 0x0000;
...
//	devinfo_data.cbSize = sizeof(SP_DEVINFO_DATA);
	...
	for (;;) {
		...
		if ((vendor_id == 0x0 && product_id == 0x0) || 
			...
			res2 = HidD_GetPreparsedData(write_handle, &pp_data);
			if (res2) {
			...
			/* Serial Number */
			res2 = HidD_GetSerialNumberString(write_handle, wstr, sizeof(wstr));
			...
			/* Manufacturer String */
			res2 = HidD_GetManufacturerString(write_handle, wstr, sizeof(wstr));
			wstr[WSTR_LEN-1] = 0x0000;
			if (res2) {
			...
			/* Product String */
			res2 = HidD_GetProductString(write_handle, wstr, sizeof(wstr));
			wstr[WSTR_LEN-1] = 0x0000;
			if (res2) {
